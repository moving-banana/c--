...
// 전역변수 input에 있는 수식의 가장 먼저 만나고 가장 깊은 괄호 하나를 풀어내고,input에 수식을 재정리하는 함수
        ...
        if (input[0] == 'e' && input[1] == 'x' && input[2] == 'i' && input[3] == 't') {
            ...
            if (hasUnknownCharacter == true) { // hasUnknownCharacter가 true일 경우(엉뚱한 값을 입력받았을 경우)
                ...
                while (1) { // 괄호를 다 풀어낼 때 까지 무한반복한다.
                    bool hasBracket = false;    //괄호가 있으면 true
                    ...
                    if (hasBracket == true ) { // hasBracket가 참일 경우(괄호를 가지고 있을 경우)
