...
int NumToKor(char* str)
{
    ...
    for (idx1 = 0; idx1 < strlen(str); idx1++) {
        ...
        tmp = str[idx1] - 48;    //ascii코드로 문자0=48
        printf("ㅋㅋ%d\n", tmp);
        count = (count * 10) + tmp;
        if (idx1 == strlen(str) - 1) printf("%s", kors[tmp - 1]);
        else {
            printf("%s%s", kors[tmp - 1], ciphers[strlen(str) - idx1 - 1]);
            if (strlen(str) - idx1 - 1 == 4) count = 0;
        }
    }
    printf("원");
    printf("\n");
